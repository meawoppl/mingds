...
import mingds.stream.GDSIIStream;
...
public class PolygonStreamTest  {
    ...
    public void testOfPolygons() throws GDSIIValidator.ValidationException, IOException {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                squares.add(Lists.newArrayList(
                        new Vector2D(i, j),
                        new Vector2D(i+1, j),
                        new Vector2D(i+1, j+1),
                        new Vector2D(i, j+1),
                        new Vector2D(i, j)
                    ).stream().map(v->v.scalarMultiply(100)).collect(Collectors.toList())
                );
            }
        }

        List<GDSIIRecord<?>> records = PolygonStream.ofPolygons(squares.stream()).collect(Collectors.toList());
        GDSIIValidator.validateRecords(records);
    ...
    public void testPolyPinwheel() throws GDSIIValidator.ValidationException, IOException {
        ...
        List<Vector2D> lumpogon = IntStream.range(0, 22).mapToObj(i -> {
            if (i % 3 == 0) {
        ...
        List<GDSIIRecord<?>> records = PolygonStream.ofPolygons(Stream.of(lumpogon)).collect(Collectors.toList());
        GDSIIValidator.validateRecords(records);
